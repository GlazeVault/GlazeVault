...
**Why:** a featured piece must always be publicly viewable AND grouped under a collection (Portfolio is rendered grouped by public collection); un-publishing or de-collecting must not leave a dangling featured piece. **How to apply:** any new surface that writes these flags must preserve the gate + coupling. Defense in depth: (a) `enforceVisibilityInvariant(piece)` in `constants/privacy.ts` is now applied centrally in `PotteryContext.normalizePiece` (on cached/remote load) AND `updatePiece` (post-merge) — it is ONE-DIRECTIONAL (`private ⇒ unfeature`, never forces public), so a private piece can never persist as featured regardless of code path; (b) render-time `isPortfolioPiece` gating still prevents exposure of any legacy orphan-featured rows. Do NOT make the invariant force `isPublic: true` from featuring — that coupling belongs to the toggle handler.
