...
**How to apply:**
- `resolvePublicOrigin()` precedence: `EXPO_PUBLIC_PUBLIC_SITE_URL` (custom-domain override) → **on web, `window.location.origin`** (the host actually serving the page right now — the most reliable source for the dominant web share flow, immune to a stale env domain) → `EXPO_PUBLIC_DOMAIN` (set by the expo `dev` workflow to `$REPLIT_DEV_DOMAIN`) → hardcoded Replit-domain last-resort (logs a `console.warn`). The `glazevault.art` brand domain is intentionally NOT the runtime fallback. Origin resolution is shared by ALL public URLs (foyer, collection, piece) by design — fixing it improves every share link, it is not scoped to the foyer.
